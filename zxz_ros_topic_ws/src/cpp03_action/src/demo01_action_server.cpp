...
public:
    ...
    */
    // 3-2.处理提交的目标值(回调函数)
    rclcpp_action::GoalResponse handle_goal(const rclcpp_action::GoalUUID & /* unused */, std::shared_ptr<const Progress::Goal> goal)
    {
        if (goal->num <= 1)
        {
    ...
    */
   // 3-3.处理取消请求(回调函数)
   rclcpp_action::CancelResponse handle_cancel(std::shared_ptr<rclcpp_action::ServerGoalHandle<Progress>> /* unused */)
   {
    ...
    void execute(std::shared_ptr<rclcpp_action::ServerGoalHandle<Progress>> goal_handle)
    {
        ...
        for (int i = 1; i <= num; i++)
        {
            ...
            if (goal_handle->is_canceling())
            {
                ...
                RCLCPP_INFO(this->get_logger(), "客户取消了任务！");
                return;
            }
            rate.sleep();
        }
